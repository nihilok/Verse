...
class InsightService:
    ...
    def save_insight(
        ...
        db.execute(user_insights.insert().values(user_id=user_id, insight_id=db_insight.id))

        db.commit()
        db.refresh(db_insight)
        return db_insight

    def link_insight_to_user(self, db: Session, insight_id: int, user_id: int) -> bool:
    ...
    def get_saved_insight(
        self, db: Session, passage_reference: str, passage_text: str
    ) -> SavedInsight | None:
    ...
    def get_user_insights(self, db: Session, user_id: int, limit: int = 50) -> list[SavedInsight]:
        """Get all saved insights for a user ordered by creation date."""
        return (
            db.query(SavedInsight)
            .join(user_insights, SavedInsight.id == user_insights.c.insight_id)
            .filter(user_insights.c.user_id == user_id)
            .order_by(SavedInsight.created_at.desc())
            .limit(limit)
            .all()
        )

    def clear_user_insights(self, db: Session, user_id: int) -> int:
        """Clear all insight associations for a user."""
        count = db.execute(user_insights.delete().where(user_insights.c.user_id == user_id)).rowcount
        db.commit()
        return count
