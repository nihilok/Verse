...
class DefinitionService:
    ...
    def save_definition(
        ...
        db.execute(
            user_definitions.insert().values(
                user_id=user_id,
    ...
    def get_saved_definition(
        ...
        passage_reference: str,
        verse_text: str
    ) -> Optional[SavedDefinition]:
        """Get a saved definition from the database."""
        return db.query(SavedDefinition).filter(
    ...
    def get_user_definitions(
        ...
        return db.query(SavedDefinition).join(
            user_definitions,
            SavedDefinition.id == user_definitions.c.definition_id
        ).filter(
            user_definitions.c.user_id == user_id
        ).order_by(
            SavedDefinition.created_at.desc()
        ).limit(limit).all()
    
    def clear_user_definitions(
        self,
        db: Session,
        user_id: int
    ) -> int:
        ...
        count = db.execute(
            user_definitions.delete().where(user_definitions.c.user_id == user_id)
        ).rowcount
        db.commit()
        return count
