...
class RagService:
    ...
    def _group_by_conversation(self, relevant_messages: list, conversation_id_field: str) -> dict[int, list]:
        """
    ...
    async def _get_relevant_messages(
        ...
        user_id: int,
        query: str,
        exclude_filter: tuple[str, int] | None,
        join_model: type | None,
        limit: int,
    ) -> list:
        """
        ...
        try:
            ...
            # Build query
            stmt = select(model_class).filter(model_class.embedding.isnot(None))
            ...
            logger.error(
                f"Error retrieving relevant messages for user {user_id}: {e}",
                exc_info=True,
            )
            return []
    ...
    def format_enhanced_rag_context(self, enhanced_contexts: list[MergedRagContext]) -> str:
        """
            ...
            for i, matched_msg in enumerate(ctx.matched_messages):
                ...
                if i < len(ctx.matched_messages) - 1:
                    ...
                    between_for_this_gap = [
                        msg
                        for msg in ctx.messages_between
