...
class ClaudeAIClient(AIClient):
    ...
    def __init__(self):
        ...
        self.client = anthropic.Anthropic(
            api_key=settings.anthropic_api_key, timeout=self.API_TIMEOUT_SECONDS
        )
    ...
    def _build_conversation_messages(self, user_message: str, chat_history: list) -> list[dict]:
        """
    ...
    async def generate_standalone_chat_response_stream(
        ...
        try:
            ...
            logger.error(
                f"Error generating standalone streaming chat response: {e}",
                exc_info=True,
            )
            raise
