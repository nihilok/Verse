...
@pytest.mark.asyncio
async def test_save_and_get_insight_with_same_text(async_db, async_test_user):
    ...
    saved = await service.get_saved_insight(
        async_db, passage_reference="John 3", passage_text="For God so loved the world"
    )
...
@pytest.mark.asyncio
async def test_different_text_same_reference_not_cached(async_db, async_test_user):
    ...
    saved = await service.get_saved_insight(
        async_db, passage_reference="John 3", passage_text="that he gave his only Son"
    )
...
@pytest.mark.asyncio
async def test_get_all_insights(async_db, async_test_user):
    ...
    insights = await service.get_user_insights(async_db, async_test_user.id, limit=50)

    assert len(insights) == 2


@pytest.mark.asyncio
async def test_clear_all_insights(async_db, async_test_user):
    ...
    count = await service.clear_user_insights(async_db, async_test_user.id)
