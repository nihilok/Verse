...
@pytest.mark.asyncio
async def test_insight_with_translation_in_reference(async_db, async_test_user):
    ...
    saved = await service.save_insight(
        async_db,
...
@pytest.mark.asyncio
async def test_different_translations_cached_separately(async_db, async_test_user):
    ...
    kjv_insight = await service.get_saved_insight(
        async_db,
        passage_reference="John 3:16 (KJV)",
        passage_text="For God so loved the world, that he gave his only begotten Son...",
    )
    ...
    web_insight = await service.get_saved_insight(
        async_db,
        passage_reference="John 3:16 (WEB)",
        passage_text="For God so loved the world, that he gave his only born Son...",
    )
...
@pytest.mark.asyncio
async def test_standalone_chat_with_translation_in_reference(async_db, async_test_user):
    ...
    # Retrieve the chat
    service = ChatService()
    chats = await service.get_standalone_chats(async_db, async_test_user.id)
...
@pytest.mark.asyncio
async def test_user_insights_with_multiple_translations(async_db, async_test_user):
    ...
    # Get user insights
    insights = await service.get_user_insights(async_db, async_test_user.id)
